...
def build_cad_masks(
    ...
    with np.load(lines_path, allow_pickle=False) as payload:
        for group, channels in CHANNEL_GROUPS.items():
            ...
            zone = polygon_mask(shape, roi_project["zones"][f"ZONE-{group}"])
            seeds = [polygon_mask(shape, roi_project["rois"][channel]) & zone for channel in channels]
            ...
            for channel, seed in zip(channels, seeds):
